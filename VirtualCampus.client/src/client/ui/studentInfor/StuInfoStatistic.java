...
public class StuInfoStatistic extends JFrame{
    ...
    {		
    	...
    	public void mouseClicked(MouseEvent arg0) 
    	{
    		...
    		studentClass = textfield.getText().trim();
    		studentSex = (String)combobox.getSelectedItem();
    		birthdayFrom = textfield1.getText().trim();
    		birthdayTo = textfield2.getText().trim();
    		
    		if(studentClass.equals("")&&studentSex.equals("性别")&&birthdayFrom.equals("")&&birthdayTo.equals(""))
    		{
                ...
                {
                  sql += " AND sClassID = "+studentClass;
                }

                if(!studentSex.equals("性别"))
                {
                  sql += " AND sSex = '"+studentSex+"'";
                }

                if(!birthdayFrom.isEmpty())
                {
                  sql += " AND sBirthday > '"+birthdayFrom+"'";
                }


                if(!birthdayTo.isEmpty())
                {
                  sql += " AND sBirthday < '"+birthdayTo+"'";
                }   
	    		rowData.removeAllElements();
	    		String[] cgColumArray = {"sCard", "sID", "sName", "sSex", "sClassID", "sBirthday", "sHometown"};
	    		ClientMsgHelper  cmh = new ClientMsgHelper();
	    		cmh.query("vcStudent", sql, cgColumArray);
	    		cmh.sendMsg();
	    		cmh.recieveMsg();
	    		cmh.disconnect();
